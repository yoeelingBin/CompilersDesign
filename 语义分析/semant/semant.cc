...
void ReturnStmt_class::check(Symbol type) {
    ...
    if (thisType != type) {
        // return type should same as decl type
    ...
    if (!isValidCallName(callName)) {
        ...
        if (callTable.find(callName) == callTable.end()) {
            ...
            for (int i=formals->first(); formals->more(i); i=formals->next(i)) {
                if (!actuals->more(k)) {
                    // when call function, parameters num must same
                ...
                if (actualType != formalType) {
                    // when call function, parameters type must same
...
Symbol Assign_class::checkType(){
    ...
    if (expectType) {
        ...
        if (sameType(actualType, *expectType)) {
            ...
            // in assignment, lvalue and rvalue type must same
            semant_error(this) << "Right value must have type " << *expectType << " , got " << actualType << '.' <<endl;
        }
        result = this->type;
    }
    else {
        // lvalue need defined before use
...
Symbol Object_class::checkType(){
    ...
    if (expectType) {
        ...
        // object must defined before use
